...
paste0("mean: ", m)
paste0("90% confidence interval: (", ci[1], ",", ci[2], ")")
```
...
paste0("Theoretical median: ",round(theoretical_median,3))
...
```
 ...
 The results show a 95% confidence interval between 24.80 and 32.97 which is a narrow confidence interval. It also shows a different confidence interval than the bootstrap confidence interval which can be explained because of the random nature of the bootstrap method. 
...
anova(chickaov)
```
From the results of one-way ANOVA we can see that the p-values is < 0.05, therefore we can conclude that at least one of the means between different feed varieties are significantly different. 
```{r, warning=FALSE}
...
table %>% 
  ...
  mutate(Type = case_when(
    ...
    knitr::kable(.)
```
From summary statistics it seems that "sunflower" feed is the feed resulting in highest weight, however by looking at the p-values we can see that it is not significantly different from "casein" feed variety, therefore we cannot conclude which one of these two is the best.
