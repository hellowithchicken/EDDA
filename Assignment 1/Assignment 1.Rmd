...
```
The graphical methods show that the data is normal. The Shapiro-Wilk test backs up this assuption as it shows a p-value of 0.8995, meaning that the H0 is not rejected and therefore the data is normal. Furthermore a point estimate for mu is conducted along side a 90% confidence interval.
...
## b)
A t-test is performed to verify the claim that the mean birthweight is bigger than 2800. The t-test shows a p-value of 0.014. This means that this claim is significant for an alpha of 0.1. The claim is fignificant for all alpha's above 0.014 and insignificant for alpha's below 0.014.
```{r}
